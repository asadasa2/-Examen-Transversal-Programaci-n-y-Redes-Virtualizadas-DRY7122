...
import uuid #for para crear identificadores universales unicos
from flask import Flask,  request 
app = Flask (__name__) # Asegurese de usar dos guiones bajos antes y despues de «nombre»
...
def signup_v1():                
    ...
    c = conn.cursor() 
    c.execute('''CREAR TABLA SI NO EXISTE USER_PLAN
...
@app .route ('/login/v2', métodos= ['GET', 'POST'])
def login_v2(): 
    error = None 
    if request.method == 'POST': 
